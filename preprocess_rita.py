...
def preprocess(args):
    ...
    if args.weight_mode == 'identity':
        ...
        for origin_airport, destination_airport in df_g.groups:
            ...
            group_mean = group_mean.fillna(df_g_means)
            features = np.array([len(group)])
...
def main():
    ...
    parser.add_argument('--edges_file',     dest='edges_file',      type=str, default='rita.cites',   help='Destination file for the edges')
    parser.add_argument('--nodes_file',     dest='nodes_file',      type=str, default='rita.content', help='Destination file for node content/features')
